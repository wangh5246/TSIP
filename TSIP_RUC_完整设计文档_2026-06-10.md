...
**结果(Rome,n=300 in-region runs,60s cadence)**:median |err| = **0.0000**,p95 = 0.0000;诊断 `zone_homogeneous_share = 1.0`(0/300 个 run 跨越多个 rate zone)。

诚实解读:在当前 proof-compatible Rome footprint 上,**诚实账单是精确的**——没有任何诚实用户因 cell 量化/cadence 被错误计费。根本原因是该工作块沿实际轨迹是 zone-homogeneous 的(官方 working block 虽含 tier 10:12 两档,但高活动轨迹聚集在单档内),60s interval 几乎不跨 zone 边界,单端点归属误差为零。

边界与方法论(必须写进论文,诚实):

- 这是当前 footprint 的性质,不是普适保证;多 zone footprint 下误差非零,上界为每段 rate spread。informative 的扩展(对 Rome 官方多边形真值测 cell-boundary 量化误差、或多 zone 合成 footprint)列为 future work。
- **过程中发现并修复了 E0 自身的一个测量 bug**:初版 coarse 账单只覆盖到末采样点、漏掉 run 尾段距离,产生虚假的单边 −7% 欠收(0/300 过收);修正为两边覆盖同一区间后误差归零。这与 E1 objective/billing drift、Baseline R 分母漂移同属"ratio 分子分母必须同源/同区间"的方法论教训,值得写进 methodology 一节作为可信度背书。

GeoLife(无地理锚点,synthetic per-cell tariff 量化误差为 0 by construction)与 Porto(原始轨迹在 ~1.7GB zip,本环境流式成本过高)跳过,receipt 有记录。
...
解读:proxy 是对 spot-check baseline 最慷慨的下界(只数 observed 路段),margin-0 真实 drivable graph 是现实宇宙;两者构成双侧夹逼,真实宇宙下 baseline 摄像头需求仍高约 2-1500 倍,TSIP-RUC 为 0。GeoLife 因旧版 `tariff_block.json` 无 block 地理锚点而跳过(receipt 有记录),北京路网证据由 T-Drive block 提供。
