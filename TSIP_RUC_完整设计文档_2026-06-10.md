...
**关键经验结果(Rome,rational omit 0.2,traffic-weighted)**:100 cams 仅 P=0.813,达到 ≥0.95 需要铺满全部 196 个 observed 段;铺满时诚实司机 route visibility = **1.0**。论文写法:
...
head-to-head 表同步量化:Baseline T 客户端计费微基准 216.5ms(plain-CPU 代理 in-enclave 工作量,measured、不含 enclave 开销,generous);Baseline Z 结构计数 ≥24 个谓词证明/period vs TSIP 单个 settlement proof。
