# TSIP-RUC(WayBill)完整设计文档

> 创建:2026-06-10;**全面整备:2026-06-12**(实验矩阵收官版——E0-E6 × 四数据集、baseline R/T/Z、Stackelberg 博弈、月度对账协议全部回填)  
> 仓库：`/Users/wanghao/Desktop/risefl_mvp`  
> 目的：把当前 TSIP-RUC(论文名 WayBill)的论文设计、协议语义、工程实现、实验进展、已知问题和下一步计划汇总成一份可继续迭代的**单一权威**设计文档。  
> 重要口径：本文档优先记录当前仓库和真实 runner 输出。历史 handoff 与早期审阅中已被推翻或修正的判断,以本文档为准;带日期的小节为增量记录,同主题以最新日期小节为准。  
> 配套:`README_ARTIFACT.md`(测试/runner/指纹速查)、两份综合审阅(06-11 与 v3)、`TSIP_RUC_论文附录素材_2026-06-11.md`。

---

## 0. 一页摘要

> **命名决定(2026-06-11)**:论文系统名定为 **WayBill**(way + bill,且 "waybill" 即运输业的计费运单术语;已查无学术撞名)。命名映射规则:
> - **论文/对外**:系统名一律 WayBill;标题示例 *WayBill: Privacy-Preserving Road Usage Charging from Authenticated Trajectory Settlement Proofs*;在一处脚注写 "builds on the spatio-temporal integrity proof (TSIP) primitive introduced in [heatmap 论文]",与第一篇论文切割并互相导流。
> - **仓库/内部**:代码、目录、receipt、本设计文档继续用 TSIP-RUC / `tsip_*` 标识,**不重命名**——已封印的 artifact 与回归测试不动;投稿写作时只在论文文本层做替换。
> - 电路/协议域分隔串(`tsip_settlement_*_v1`)保持不变:它们是已部署接口的一部分,论文中可注明 "domain separators retain the tsip_ prefix for artifact continuity"。

TSIP-RUC(论文名 **WayBill**)是把 TSIP 的时空完整性证明思想迁移到道路使用计费（Road Usage Charging, RUC）的隐私保护结算系统。核心目标不是做位置聚合，而是让每个车辆在不上传完整轨迹的前提下提交一份可验证账单：账单必须按公开 tariff 表正确计算，轨迹必须满足认证时间、速度、连续性、里程和区域计费约束。

论文 thesis 建议谨慎写成（系统名按命名决定用 WayBill;**2026-06-11 v3 审阅修订:三元表述可被 TEE 一句话击杀——TEE 同样满足三项——必须把 policy-free TCB 升入 thesis 本体**）：
...
**当前状态快照(2026-06-12,实验矩阵收官)**——七组实验 × 四数据集全部有 receipt,baseline 三件套(R/T/Z)全部量化,协议层完整性缺口(选择性不提交)已修复,所有结论可追溯到 `experiments/` 下的权威目录(快速索引见 `README_ARTIFACT.md`):

- **E0 诚实账单准确性**(§9.5):Rome/Porto 在 proof-compatible footprint 上账单**精确**(zone-homogeneous);GeoLife 多档情形下误差双侧近无偏(mean +0.26%,median 0,p95 |err| 10.7%),是量化噪声而非系统偏收。
- **E1 约束消融**(§10):GeoLife 主 anchor(no_zone_binding ε median 0.276),Rome 官方 tariff 辅证,四数据集 pinned OSNMA replay 全部 ε=0。**tariff-spread 曲线**(§10.4.1)证明 ε 是 tariff 累进度的单调函数(1.2× 时 0.047 → 10× 时 0.309),化解"合成 tariff artifact"质疑。
- **E2 fallback 机制**(§11):α≥1 充分界 + 经验 break-even 分布;GeoLife 运营点 α=0.6/0.4,Porto 同方向(0.5/0.4),四数据集覆盖。
- **E3 proof-consistent relay 残余**(§12):四数据集全量 sweep(`e3_relay_residual_all4/`),最坏格点 Rome fine/intra_cell/400m **p95 0.507(保守上界)**;50m bucket 敏感性证实上界宽松(同格点 Rome p95 0.464→0.295),60s cadence 把最坏 coarse 残差压到 0.187。
- **E4 执法对比**(§13):双宇宙(observed proxy + margin-0 真实路网)+ 检出曲线 + 等检出工作点 + **Stackelberg 博弈**(对知道摄像头位置的自适应瞒报者,任何部分覆盖检出率恒 0;全覆盖时诚实路线可见度 1.0)+ R/T/Z head-to-head + 交叉对称评估(同攻击打 TSIP 必亏钱:GeoLife 多付 3.2-3.8×)。真实路网摄像头需求:Rome 364@omit0.2、北京 GeoLife 网格 **~17,000** vs WayBill **0**。
- **E5 组合侧信息泄漏**(§14):Rome 162 anchor(zone-priced median anonymity 3 / odometer-only 28);泄漏光谱(GPS 1.0 / spot-check 等检出点 1.0 / proof-only 0.0);statement-bucketing 缓解曲线(exact median 1 → 50% 网格 3);**争议仲裁泄漏**(单次 level-2 开仓去匿名化 76%,level-1 应为默认)。
- **E6 性能**(§15):snarkjs prove 7.13s + **rapidsnark 实测 median 1.389s**(双 measured,extrapolated 标注作废);60s cadence 全月 prove ≈208s;verify 223-456ms;proof 709-805B,21 public signals。

**协议层完整性(2026-06-12 重大补全,§3.3)**:v3 审阅发现选择性不提交攻击(整 period 不提交 ε=1.0),已用**月度 odometer 对账**修复——receiver 签名月界里程读数,未覆盖里程按 ≥α·max_rate 计费,得到 monthly revenue soundness(no-free-kilometer)保证;monotone degradation 补全为四层。charger 同时改为 `(device_id, period_id)` 键、分设备账本、period 时间窗不重叠检查。
...
- charger 的 `/settlement/period`(raw-fix)路径仅为工程验收;隐私部署路径是 `/settlement/period/proof-only`(已实现,负例测试齐全)。
- Porto/T-Drive 小样本(10/1 periods)是 256-cell footprint 的结构性产出限制,只作 robustness/coverage,不作 anchor;样本框统一说明见 §9.1。
- 测量方法论三训(写进论文 methodology):对手目标函数与计费必须同源(E1 drift);ratio 分子分母必须同源同区间(Baseline R 分母、E0 尾段两次修复);generous 参数须检查偏置方向(TEE 微基准、drivable margin 两次修复)。
...
- **选择性不提交(2026-06-11 v3 审阅 GAP-1,已修复)**:只为便宜行程提交 period proof,贵行程整段不提交。防御:月度 odometer attestation 对账——未被 accepted periods 覆盖的里程按 `reconciliation_rate ≥ α·max_zone_rate` 计费,瞒报整 period 不可能省钱(E2 单调降级的月级扩展);同设备 period 时间窗不重叠检查防止虚增覆盖里程。
...
当前主设计采用 **direct accumulation + monthly reconciliation**（2026-06-11 v3 审阅后升级）：

1. 每个 period 独立生成 proof。
2. Charger 验证 proof 与 public statement。
3. 按 `(device_id, period_id)` 去重,并检查同设备 period 时间窗 `[period_start, period_end)` 不重叠。
4. 将 `total_fee_cents`、`total_distance_m`、`fallback_intervals` 累加到 `monthly_ledger[(device_id, month_id)]`(分设备账本)。
5. **月度对账(completeness)**:receiver 对 `(device_id, month_id, odometer_start_m, odometer_end_m)` 出具签名 attestation(`POST /settlement/month/attest`,域分隔 `tsip_settlement_monthly_odometer_attestation_v1`);月末 `GET /settlement/month/{month_id}/reconcile` 计算 `unaccounted_m = attested_delta − Σ accepted_distance`,按 `RECONCILIATION_RATE_CENTS_PER_M`(配置,须 ≥ α·max_zone_rate, α≥1)计费。由 odometer 时间单调性 + 时间窗不重叠,Σ period distance ≤ attested delta 对诚实设备恒成立;负值即标记不一致。

由此获得月级保证(论文可写成命名结果):

> **Proposition (monthly revenue soundness / no-free-kilometer).** Under odometer-attested reconciliation with rate ≥ α·max_zone_rate (α ≥ 1), the accepted monthly bill is at least the tariff-true fee of all driven kilometres, whether or not the user submits period proofs; truthful full submission is the cost-minimizing strategy.

证明 = E2 四行不等式 + odometer 单调性 + 对账等式。新增公开信息只有月度里程总量(odometer-only RUC 本就公开该字段),写进 E5 的 `L_month = L ∪ {odo_month_delta}`。Monotone degradation 由此补全为四层:missing fix → interval fallback;missing interval → period fallback;**missing period → monthly reconciliation**;missing everything → administrative path。
...
### 4.4 双提交路径(原"隐私差距",proof-only 路径已实现后改写)

charger 现有两条提交路径:

- `/settlement/period`(**工程验收路径**):接收 raw `fixes`,用于设备签名验证、public statement recomputation、submission gate 负例测试。证明 settlement correctness,**不作为 privacy claim 依据**。
- `/settlement/period/proof-only`(**隐私部署路径,已实现**):只接收 public statement + Groth16 proof + public signals + receiver root attestation;Pydantic `extra="forbid"` 拒绝夹带 `fixes`;charger 验 vkey、signal layout、statement commitment、canonical month、registry DAC、root attestation。负例测试覆盖 signal drift / 坏 attestation / 夹带 fixes。

论文所有 privacy claim 以 proof-only 路径为对象;月度对账(§3.3)在两条路径之上统一生效。
...
- `POST /settlement/period/proof-only`
- `POST /settlement/month/attest`(月度 odometer attestation,2026-06-11 新增)
- `GET /settlement/month/{month_id}/reconcile?device_id=`(月度对账,2026-06-11 新增)
- `GET /settlement/month/{month_id}`(可选 `device_id` 查询参数;不带时返回跨设备聚合)
...
- `(device_id, period_id)` 去重 + 同设备 period 时间窗不重叠检查
- 分设备 monthly ledger 累加
- 月度 odometer attestation 验签与对账(completeness,§3.3)
...
| 实验 | 目的 | 当前状态(2026-06-12) | 权威目录 |
|---|---|---|---|
| E0 | 诚实账单准确性 vs 连续 GPS 真值 | 已完成三数据集(§9.5):Rome/Porto 精确,GeoLife 多档双侧近无偏 | `e0_billing_accuracy/` |
| E1 | 完整性组件经济消融，回应 desk reject | 已完成 + tariff-spread 曲线(§10.4.1) | `e1_objective_fix/`、`e1_rome_real_tariff_ratio_fix/`、`e1_tariff_spread/` |
| E2 | fallback rate 的机制设计 trade-off | 已完成增强版 + 四数据集覆盖(§11.3.1) | `e2_fallback_frontier/`(anchor)、`_all4/` |
| E3 | proof-consistent relay / adversarial residual | 已完成四数据集全量 sweep + 50m bucket 敏感性 | `e3_relay_residual_all4/`、`e3_bucket_sensitivity/` |
| E4 | 路侧基础设施成本 vs spot-check baseline | 已完成:双宇宙、检出曲线、Baseline R 仿真器、Stackelberg、R/T/Z、交叉对称 | `e4_e5_ruc/` |
| E5 | 组合侧信息泄漏 | 已完成:anonymity(四数据集)+ 泄漏光谱 + bucketing 缓解 + 仲裁泄漏 | `e4_e5_ruc/` |
| E6 | 性能与规模 | snarkjs 7.13s + rapidsnark 1.389s 双实测;direct accumulation 月度表为 measured | `e6_rapidsnark/` |

最小可发表主线仍是 E1 + E2 + E3;E4(执法博弈)与 E5(泄漏光谱)是外部 head-to-head 的关键;E0 回应 transport reviewer 的准确性质疑;E6 回应可部署性。

### 9.1 样本框统一说明(2026-06-11 审计补充)

各实验的 n 不同来自**不同的合法过滤器**,论文与 artifact 必须按此口径报告,避免被误读为不一致:

| 数据集 | 加载总数 | E1/E2/E3 口径 | E4/E5/Baseline R 口径 |
|---|---:|---|---|
| GeoLife | 53 | 14(`geolife_periods=14`,E1 cadence-clean anchor) | 53(全部加载 periods);E5 anonymity pool=53,Baseline R movement pool=53 |
| Rome | 162 | E1/E2 162;E3 66(`min_distance_m=500` moving 过滤) | E4/E5 162;Baseline R / cross-system movement pool=123(有移动段的 periods) |
| Porto | 10 | E1/E2 10;E3 8(moving 过滤) | 10;movement pool 9 |
| T-Drive | 1 | E1/E2 1;E3 0(被 moving 过滤) | 1 |

规则:任何对比表内的两个系统必须用**同一池**(§13.8 C 的 same-pool 重算即为此);跨实验引用时注明池定义。

---

## 9.5 E0 诚实账单准确性（2026-06-12,v3 审阅"必做"项）

E1-E5 全部回答"对手能少付多少",E0 回答 transport reviewer 的第一个问题:**诚实用户的账单相对连续 GPS 真值差多少?** runner `script/run_e0_billing_accuracy.py`,输出 `experiments/e0_billing_accuracy/`。

方法:协议把每个 cadence interval 按起点 fix 的 zone 计费、覆盖整段 odometer 距离;细粒度真值对每个原始 GPS 步按其所在 cell 的 zone 计费。两者覆盖完全相同的 `[首采样点,末采样点]` 区间(总距离相同),相对差 `(coarse−fine)/fine` 隔离出 cadence 下采样 + 单端点归属的误差。复用生产投影/tariff block/raw-point 迭代器,口径与 charger 一致。

**结果(2026-06-12 三数据集全量,各 n=300 in-region runs,60s cadence,trip 级流式)**:

| Dataset | median \|err\| | p95 \|err\| | mean signed | 跨 zone run 占比 | 解读 |
|---|---:|---:|---:|---:|---|
| Rome | 0.0000 | 0.0000 | −0.0000 | 0/300 | footprint 沿轨迹 zone-homogeneous,账单精确 |
| Porto | 0.0000 | 0.0000 | −0.0000 | 0/300 | 同上 |
| **GeoLife** | 0.0000 | **0.1065** | **+0.0026** | **164/300(54.7%)** | 真正的多档情形:误差双侧(64 过收/80 欠收,max +0.388/−0.204),mean 近零 → 量化噪声而非系统性偏收 |

**GeoLife 锚点复原(2026-06-12,重要)**:GeoLife 的 geometry 来自内置北京中心网格 `tariff_at_granularity('beijing','medium',grid_w=100)`(10,000 cells,origin 39.9042/116.4074,`cell = floor((x+5000)/100)`),重新生成的 `cell_zones` 与存档 tariff **逐字节一致**,998/998 fixes 命中——锚点不是丢失而是内置常量。这同时解锁了 E0-GeoLife 与 E4 真路网-GeoLife(见 §13.5)。

E0 论文写法:

> On zone-homogeneous footprints the honest bill is exact. On the multi-zone Beijing grid, 55% of runs cross rate boundaries; the per-run attribution error is two-sided with near-zero mean (+0.3%), median zero, and p95 |error| 10.7% — quantization noise rather than systematic over- or under-charging. Cell size and cadence bound this error and are the protocol's accuracy knobs.

方法论(必须写进论文,诚实):**过程中发现并修复了 E0 自身的一个测量 bug**——初版 coarse 账单只覆盖到末采样点、漏掉 run 尾段距离,产生虚假的单边 −7% 欠收;修正为两边覆盖同一区间后 Rome 误差归零、GeoLife 呈双侧近无偏。这与 E1 objective/billing drift、Baseline R 分母漂移同属"ratio 分子分母必须同源/同区间"的方法论教训。Porto 经 trip 级流式处理已纳入;T-Drive 无本地原始轨迹文件,receipt 有记录。
...
### 10.4.1 E1 tariff-spread sweep（2026-06-12,化解"0.27 是合成 tariff 制造"质疑）

v3 审阅指出:GeoLife no_zone_binding ε≈0.27 与 Rome ≈0.006 相差 48 倍,部分由 tariff 极差驱动(GeoLife 合成 `{1,3,5}` 极差 5×,Rome 官方 working block 仅覆盖 `10:12` 极差 1.2×),reviewer 可能质疑 0.27 是陡 tariff 的 artifact。`script/run_e1_tariff_spread.py`(复用 `zone_binding_min_fee` + `compute_bill`)固定同一组 GeoLife 轨迹与 zone 拓扑,只变费率极差,输出 ε vs spread 曲线(`experiments/e1_tariff_spread/`,n=50 periods):

| tariff spread | rates | median ε | p95 ε |
|---:|---|---:|---:|
| 1.2× (Rome-like) | 10\|11\|12 | 0.0473 | 0.0580 |
| 2× | 10\|15\|20 | 0.1540 | 0.1843 |
| 3× | 10\|20\|30 | 0.2144 | 0.2531 |
| 5× (GeoLife 实际) | 10\|30\|50 | 0.2668 | 0.3113 |
| 10× | 10\|55\|100 | 0.3088 | 0.3568 |

两个关键结论:(a) spread=5× 的 median 0.267 **复现权威 E1 GeoLife 数字 0.276**,验证实现一致;(b) 把 GeoLife 轨迹放到 Rome 的 1.2× 极差下,ε 降到 0.047——接近 Rome 的 0.006 量级,说明 **0.27 不是合成 tariff 的 artifact,而是 tariff 累进度的单调函数**。政策级 finding:

> Zone-binding 约束的经济价值随 tariff 累进度单调增长;计费结构越平坦,需要的密码学完整性越少。城市设计 tariff 时,rate spread 同时决定了完整性约束的边际价值。

这把 Rome 的低 ε 从"辅证偏弱"重新定位为曲线上的一个点,给 E1 一个第三方无法指责的解释框架。
...
**Distance-bucket 敏感性(2026-06-12,可选增强落地)**:`experiments/e3_bucket_sensitivity/` 在 medium/120s/200m/intra_cell 格点用 50m bucket 重跑:

| bucket | GeoLife p95 / max | Rome p95 / max |
|---|---|---|
| 100m(all4 主口径) | 0.162 / 0.188 | 0.464 / 0.536 |
| 50m(细化) | 0.140 / 0.158 | **0.295 / 0.349** |

细 bucket 残差单调下降(Rome p95 −36%),证实两点:(a) bucketed RCSPP 上界语义成立——bucket 越粗上界越松;(b) **主表的 p95 0.507 是保守上界**,真实 proof-consistent 暴露更低。论文写法:把 100m bucket 数字作为 conservative upper bound 报告,50m 敏感性作为 bound tightness 证据,这让 E3 的 upper-bound 口径有了实测支撑而非口头声明。
...
2026-06-11 上午状态（**已被 §13.5-13.8 全面取代,保留为历史记录**）：

- PBF -> 可驾驶路网 graph loader 当时未完成(缺 `osmnx`/`pyrosm`);**同日下午已装齐依赖并落地**(§13.5),margin-0 真实路网四数据集齐全(含 06-12 恢复锚点后的 GeoLife)。
...
95% 检出目标下,真实路网宇宙 vs observed-path generous proxy(**2026-06-11 深夜修正:margin 改为 0**——此前 200m margin 把 footprint 外的路算进宇宙,虚增 baseline 摄像头数,违反 generous 纪律;旧数字 Rome 973/Porto 760/Beijing 2708 作废):

| Dataset | footprint 路网 km | 100m 段数 | omit=0.20 cams@P=0.95(真实) | omit=0.10 cams(proxy) | 备注 |
|---|---:|---:|---:|---:|---|
| Rome | 58.2 | 582 | 364 | 196 | omit=0.10 时 P=0.95 不可达(全覆盖 max P=0.909) |
| Porto | 52.0 | 520 | 325 | 42 | 同上不可达口径 |
| Beijing (T-Drive block) | 232.8 | 2328 | 1453 | 1 | 同上 |
| **Beijing (GeoLife 10×10km 网格)** | **1695** | **16953** | ~16953(S=15 时近全覆盖) | 370 | 2026-06-12 锚点复原后新增;100 km² footprint |

解读:proxy 是对 spot-check baseline 最慷慨的下界(只数 observed 路段),margin-0 真实 drivable graph 是现实宇宙;两者构成双侧夹逼,真实宇宙下 baseline 摄像头需求仍高约 2-1500 倍,TSIP-RUC 为 0。GeoLife 锚点已于 2026-06-12 复原(§9.5),其 footprint 是完整 10×10km 中心城区网格——**约 1.7 万个路侧摄像头 vs TSIP 0**,是 E4 最强的单行证据(注意 footprint 面积与其他三行不同,引用时注明)。
...
**新的诚实发现(必须进论文)**:在该检出模型下,Rome omit=0.10、S=24 时 `f·S=2.4`,全覆盖也只能到 `P=0.909 < 0.95`——即对小比例瞒报,spot-check 在任何摄像头数量下都达不到 proof-equivalent 检出;95% 目标只在 omit=0.20 时可达(margin-0 真实路网 364 cams)。此前 §13.4 proxy 表把"coverage 截断到 1.0"报成达标,口径已由 `reachable_at_full_coverage` 修正。论文写法:
...
(**2026-06-11 深夜分母修正**:省钱比分母从"移动段费用"改为**全账单**(`period_full_fee`,含 cell 内移动)。Rome cell 级抽象下移动段费用只占全账单 14.7%(receipt 的 `movement_fee_share_of_full_bill`),旧分母把 Rome 省钱比虚高约 7 倍——与 E1 objective/billing drift 同类风险;GeoLife 移动占比 96.6%,基本不变。修正前 Rome 数字 0.406/0.144/0.017 作废。)
...
| 自适应瞒报的不可检出省钱比(占**全账单**,fee-weighted 摆位下) | 0.060 | 0.011 | 0.002 | 0.0 |

GeoLife 对照(移动费占比 96.6%,自适应省钱比依然显著):10 cams 0.424、100 cams 0.300、350 cams 0.016——结构性结论(部分覆盖必有不可检出正收益)在两个数据集都成立,Rome 的绝对量级受 cell 级抽象的移动费占比限制,论文写作时两个数据集都报并给出 `movement_fee_share` 口径。
...
**A) 同一瞒报攻击打 TSIP**(与 Baseline R 完全相同的策略与 seed):savings 全部 ≤0——GeoLife 瞒报者平均多付 3.18-3.76 倍(fallback 按 `ceil(dt·vmax)·max_rate` 计),Rome 平均多付 18-21%,max savings 恒 0,无 period 触发 `max_dt` outage 边界。**同一策略、同一全账单分母:打部分覆盖 spot-check = 不可检出正收益(§13.7,GeoLife 最高 0.42、Rome 最高 0.06);打 TSIP = 严格亏损。**这是 E2 单调降级在 head-to-head 语境下的直接展示。
...
**C) 隐私同尺子且同池**:用 E5 empirical anonymity set 度量 spot-check 观测者视角(只看段、不看时间戳,偏向 baseline),并把 TSIP proof-only 在**同一个 123 moving-period 池**上重算以消除池规模偏差:TSIP zone-priced median 3 / p95 6;spot-check 等检出覆盖(196 cams)median **1** / p95 24,50 cams 下 median 5。**同一度量、同一池下,spot-check 工作点的隐私严格差于 TSIP proof-only。**
...
**已全面量化(见 §14.3-14.6),本节保留为设计时的注意事项清单**:
...
| Spot-check 谱系(等检出工作点) | **1.0**(Rome/GeoLife/Porto 均接近或达到铺满) | Rome 196 / GeoLife 350(细网格修正,旧值 370)/ Porto 42 cams |
...
### 14.5 statement-bucketing 缓解（2026-06-12,把"我们有缓解"从口头变测量）

v3 审阅要求把 fee bucketing 从 discussion 升级为 measured。`script/run_e5_fee_bucketing.py`(复用 E5 record loader)以 **exact statement 为真实泄漏基线**(协议实际公开精确 `(fee, distance, fallback, intervals)`,比 §14.3 的 ±10% 容差口径更悲观也更诚实),对 fee 与 distance 按数据集中位数的相对网格联合粗化(月末精确结算余量),输出 `experiments/e4_e5_ruc/e5_statement_bucketing.csv`:

| grid(占中位数) | Rome median / p95 | GeoLife median / p95 |
|---|---:|---:|
| exact(真实泄漏) | 1 / 1 | 1 / 1 |
| 5% | 1 / 2 | 1 / 2 |
| 10% | 1 / 3 | 1 / 4 |
...
结论:精确 proof-only statement 高度可识别(median anonymity = 1);statement bucketing 单调恢复匿名性(Rome 50% 网格 median 升到 3),代价是延迟结算精度。这与 §6.4(deferred-disclosure settlement,v6 路线图)正交互补:bucketing 是同档位下的近期缓解,deferred disclosure 是去掉时序泄漏的远期缓解。E5 一章由此从"诚实承认泄漏"升级为"度量泄漏 + 两级缓解路线图"。

注:该曲线用 exact 基线,数值比 §14.3 的容差版(Rome median 3)更小,二者口径已注明,投稿时统一以 exact 为 worst-case、容差版为 robustness。

### 14.6 争议仲裁泄漏量化(2026-06-12,审阅 §3.6 落地)

`script/run_e5_dispute_leakage.py`:观察者持 proof-only 元组(E5 容差口径)+ k 次 level-2 开仓(开出争议 interval 两端 fix 的 cell@seq),被开 interval 按"费用最高优先"选(charger 实际会争议的段)。输出 `experiments/e4_e5_ruc/e5_dispute_leakage.csv`:

| k(开仓数) | Rome median | p95 | 完全识别占比 |
|---:|---:|---:|---:|
| 0(=E5 基线) | 3 | 7 | 19% |
| 1 | 1 | 2 | **76%** |
| 2 | 1 | 2 | 81% |
...
结论:**单次 level-2 仲裁开仓即把 76% 的 period 在池内完全去匿名化**。这给出两条协议设计含义:(a) level-1 开仓(只开计费元组、不开 cell)应为默认仲裁模式,level-2 仅在 level-1 无法裁决时使用;(b) 仲裁泄漏是一次性、局部化的(只影响被争议 period,不影响其他 period 的匿名性)。论文 dispute 一节由此从"协议存在"升级为"协议存在 + 泄漏代价已量化 + 分级使用建议"。
...
| 文件 | 作用 |
|---|---|
| `common/settlement.py` | ReceiverFix、签名、commitment、tariff Merkle、fee calculation、public statement、submission verification、**月度 odometer attestation(sign/verify,2026-06-12)** |
| `common/dispute.py` | 争议仲裁选择性披露：interval opening builder / verifier / disclosure profile |
| `services/charger/app.py` | FastAPI charger:device registry、双提交路径(raw-fix 工程版 + proof-only 隐私版)、**(device_id, period_id) 键 + 分设备账本 + 时间窗不重叠 + 月度对账 `/month/attest`、`/month/{m}/reconcile`** |
| `circuits/settlement_period_v5_base.circom` | Settlement circuit base(226,394 constraints) |
| `circuits/settlement_period_v5_k6.circom` | Compiled profile wrapper(21 public signals,接口已封口) |
| `script/prove_settlement_period_v5.py` | witness/proof/verify/charger check driver,`PUBLIC_SIGNAL_ORDER` 权威定义 |

### 16.2 Evaluation runners(全量)

| 文件 | 作用 | 喂养 |
|---|---|---|
| `common/eval_harness.py` | E1/E3 RCSPP solver、constraint checker、billing wrapper | E1/E3 |
| `common/eval_experiments.py` | E2/E3/E4 helpers(`e4_required_cameras` 检出模型) | E2-E4 |
| `script/run_e0_billing_accuracy.py` | 诚实账单 vs 连续 GPS 真值(trip 流式,三数据集) | E0 |
| `script/run_e1_forensic.py` | E1 消融 runner、receipts、forensic CSV | E1 |
| `script/run_e1_tariff_spread.py` | zone-binding ε vs tariff 极差曲线 | E1 |
| `script/run_e2_fallback_frontier.py` | E2 alpha/frontier/break-even(四数据集) | E2 |
| `script/run_e3_relay_residual.py` | E3 relay/meaconing residual sweep | E3 |
| `script/run_e4_e5_ruc_experiments.py` | E4 proxy + E5 anonymity/graph-count + fraud metrics | E4/E5 |
| `script/build_e4_drivable_graph.py` | 真实 drivable graph 路网宇宙(osmium+pyrosm,四城含 GeoLife 网格) | E4 |
| `script/run_e4_baseline_comparison.py` | 检出曲线、等检出工作点、R/T/Z head-to-head、TEE 微基准 | E4 |
| `script/run_baseline_r_simulator.py` | Baseline R 经验仿真(Monte Carlo + Stackelberg 双最优反应) | E4/E5 |
| `script/run_cross_system_fairness.py` | 交叉对称:同攻击/同成本/同隐私度量打两个系统 | E4/E5 |
| `script/run_e5_fee_bucketing.py` | statement-bucketing 匿名度缓解曲线 | E5 |
| `script/run_e5_dispute_leakage.py` | k 次仲裁开仓的匿名度收缩 | E5 |
| `script/bench_rapidsnark.py` | rapidsnark prover 实测 | E6 |
| `script/build_rome_parking_tariff_geojson.py` | Rome 官方 tariff importer | 数据 |

### 16.3 权威结果目录(完整)

| 目录/文件 | 内容 |
|---|---|
| `experiments/e0_billing_accuracy/` | E0 三数据集 + zone-homogeneity 诊断 |
| `experiments/e1_objective_fix/` | E1 四数据集权威结果 |
| `experiments/e1_rome_real_tariff_ratio_fix/` | Rome 官方 tariff 权威结果 |
| `experiments/e1_tariff_spread/` | ε vs spread 曲线 |
| `experiments/e1_E1_FINAL_README.md` | E1 可引用结果 memo |
| `experiments/e2_fallback_frontier/` + `_all4/` | E2 anchor + 四数据集 |
| `experiments/e3_relay_residual_all4/` | E3 权威(`_full/` 一致性自检,首版为样本) |
| `experiments/e3_bucket_sensitivity/` | 50m bucket 上界紧度 |
| `experiments/e4_e5_ruc/` | E4/E5 全家:proxy、drivable、检出曲线、工作点、head-to-head、Baseline R 仿真、泄漏光谱、bucketing、仲裁泄漏、cross-system、fraud metrics(各自带 receipt) |
| `experiments/e6_rapidsnark/` | rapidsnark 实测 receipt |
| `dataset/osm/zones/README_rome_tariff.md` + `rome_medium.geojson` | Rome tariff 方法与 overlay |

**不要引用**:`e1_sweeps/`(pre-fix 0.94)、`e1_rome_real_tariff_smoke/`(旧 mapping)、被删除的初版 `e5_fee_bucketing.csv`(绝对桶宽语义缺陷,已被 `e5_statement_bucketing.csv` 取代)。

### 16.4 Tests(2026-06-12 全量)

默认 `pytest`(42 项,秒级):`test_charger_service.py`(14,含月度对账正负例)、`test_settlement_architecture.py`、`test_dispute_opening.py`(14)。

扩展评估(显式指定,~50 项):`test_eval_harness.py`、`test_eval_experiments.py`、`test_run_e1_forensic.py`、`test_run_e4_e5_ruc_experiments.py`、`test_run_e4_baseline_comparison.py`(5)、`test_run_baseline_r_simulator.py`(8,含 Stackelberg 语义与 omission 计费单调性)、`test_build_rome_parking_tariff_geojson.py`、`test_trajectory_preprocess.py`。

Proof 工具链(需 node/snarkjs,~96s):`test_prove_settlement_period_v5.py`(9,wrapper/prover/artifact/charger 五方一致性——改 public signal 顺序前必跑)。

命令速查见 `README_ARTIFACT.md` §1。

---

## 17. 论文写作主线

### 17.1 Introduction 核心叙事(2026-06-12 终版)

1. **政策时间窗(五线交汇 + 第六线能力勘误 + 第七线 completeness 重读)**:计费需求爆发 × 隐私滥用进入诉讼 × 立法收紧 × 认证硬件法定普及(Smart Tacho 2,2026-07 扩至轻型商用车;Singapore ERP 2.0 即 policy-protected GPS 的全国活体)× 学术方案停在 P4TC(2020)。
2. **能力勘误**(第二段,配 Pareto 图):文献默认 spot-check 检出率是预算问题;经验仿真证明对知道摄像头位置的对手,任何部分覆盖检出率为 0,全覆盖即隐私归零——spot-check 在结构上买不到 proof rejection 卖的东西。
3. **completeness 重读**:十五年的路侧基建买的主要是 completeness 而非 correctness;WayBill 把两维分离(proof + odometer 对账),路侧在两维上都不再必要(2×2 框架图)。
4. WayBill 协议:zk proof 证明"账单来自认证且物理可行的隐私轨迹",月度对账保证 no-free-kilometer。
5. 贡献:见 §17.7 v4 六条(trilemma+policy-free TCB / ε-economic soundness+spread 曲线 / 端到端激励兼容降级+月度 revenue soundness / enforcement game / settlement-metadata leakage+仲裁泄漏 / artifacts)。
6. Oregon 事实口径警告:写 "legislated mandatory phase-in (EVs from 2027)",引 ODOT/NCSL,勿用"2026-04 已强制"(媒体口径,错误——v3 审阅 §3.4)。
...
- **Monthly revenue soundness(2026-06-12 新增)**:在月度 odometer 对账下(rate ≥ α·max_zone_rate, α≥1),无论用户提交多少 period,月账单 ≥ 全部已驶里程的 tariff-true 费用;完整提交是唯一成本最优策略(coverage-DSIC)。见 §3.3 proposition。

### 17.3 Privacy claim 边界

论文必须清楚区分：

- 目标协议公开：fee、distance、fallback count、period/month metadata、roots,**以及月度 odometer 总里程(对账新增,odometer-only RUC 本就公开该字段)——`L_month = L ∪ {odo_month_delta}`**。
- 不公开：完整轨迹、zone histogram、intermediate timestamps、per-interval fees。
- 当前 artifact charger 上传 fixes 是 engineering validation path，不是最终 privacy path(proof-only 路径已实现)。
- 仲裁泄漏分级:level-1 开仓只披露计费元组;level-2 披露两端 fix——单次 level-2 去匿名化 76%(§14.6),协议默认 level-1。

### 17.4 Evaluation claims(2026-06-12 终版)

E0：

> On zone-homogeneous footprints the honest bill is exact. On the multi-zone Beijing grid, 55% of runs cross rate boundaries; the per-run attribution error is two-sided with near-zero mean (+0.3%), median zero, and p95 |error| 10.7% — quantization noise rather than systematic over- or under-charging.

E1：

> Constraint ablation exposes marginal economic attack surface; OSNMA-pinned claims block lifted replay across datasets. The zone-binding epsilon grows monotonically with tariff rate spread (0.047 at 1.2x to 0.309 at 10x on identical trajectories): flatter tariffs need less cryptographic integrity, and the rate structure itself sets the marginal value of the integrity constraints.
...
> Even under proof consistency, relay capability, tariff granularity, and intra-cell payload uncertainty determine a measurable residual economic exposure; in the full bucketed RCSPP sweep, the residual reaches p95 0.507 only in the Rome fine/intra-cell/400m upper-bound setting, and a 60s cadence cuts the worst Rome coarse-grid residual from p95 0.450 to 0.187. A finer 50m bucket lowers the same grid point from 0.464 to 0.295, confirming the bound is conservative.

E4：

> Against a placement-aware omitter, any partial camera coverage yields zero detection (per-period and cumulative) with quantified undetected savings; closing evasion requires full coverage, at which point every honest route is fully visible. The same omission strategies replayed against WayBill are strictly unprofitable (GeoLife pays 3.2-3.8x more). Proof rejection is the only point in this design space with deterministic detection, zero infrastructure, and zero route visibility — on the central-Beijing grid the spot-check equivalent is ~17,000 roadside cameras.

E5：

> The proof-only statement is not leakage-free: the exact per-period tuple has median anonymity 1; statement bucketing restores it monotonically (Rome median 3 at a 50% grid), the odometer-only tier reaches 28, and a single level-2 dispute opening de-anonymizes 76% of periods — so level-1 openings are the default arbitration mode.

E6：

> Period proof cost is measured at 1.39s (rapidsnark) / 7.13s (snarkjs) per 2h period; a full month at 60s cadence costs ~208s of client CPU, generated in the background while driving.
...
AttestMonth(device, month_id, odo_start, odo_end) -> odometer_attestation   // 2026-06-12 新增
AggregateMonth({accepted statements}, odometer_attestation) -> bill          // 含对账:未覆盖里程按 fallback 计费
Dispute(statement, indices, openings) -> accept/reject                       // level-1 计费元组 / level-2 含 fix
```
...
### 17.7 Contribution 列表(v4,2026-06-12 终版;历史 v1-v3 见创新性补充文档与两份综合审阅)
...
> 2. **ε-economic soundness.** We introduce a quantitative economic security metric for verifiable billing and a receipt-auditable measurement methodology (constraint ablation with a shared billing oracle), exposing per-constraint economic attack surface on four real mobility datasets — including a measured ε–tariff-spread curve that tells tariff designers how much cryptographic integrity their rate structure actually buys.
> 3. **Incentive-compatible degradation, end to end.** We prove sufficient conditions for dominant-strategy truthful reporting at every degradation layer — per-interval outage (α ≥ 1) and whole-period withholding (odometer-reconciled monthly settlement) — yielding a monthly revenue-soundness guarantee: no driven kilometre is ever billed below, or hidden from, the tariff. Empirical break-even distributions then locate operating points well below the conservative bound.
> 4. **Enforcement game.** We model spot-check enforcement as a Stackelberg game on real trajectories: against a placement-aware omitter, any partial camera coverage yields zero detection (per-period and cumulative) with quantified undetected savings; full coverage closes evasion but makes every honest route visible. Proof rejection is the only point in this design space with deterministic detection, zero infrastructure, and zero route visibility.
> 5. **Settlement-metadata leakage + dispute disclosure.** We give the first side-channel analysis of proof-based RUC public statements (anonymity sets, cadence/bucketing tradeoffs) including a selective-disclosure dispute protocol whose per-arbitration leakage is itself measured (one level-2 opening de-anonymizes 76% — level-1 is the default).
> 6. **Implementation & artifacts.** A 226k-constraint Groth16 settlement circuit (1.39s prove measured), charger service with monthly reconciliation, and a fully receipted evaluation harness over four mobility datasets.

对应论文定位句:

> WayBill is a settlement protocol for zone-based road usage charging that shifts enforcement from roadside observation to proof verification. Its main trade-off is explicit: trusted sensing remains in the receiver/odometer layer, while tariff policy, physical feasibility, period binding, completeness reconciliation, and billing arithmetic are verified outside that TCB.
...
**2026-06-11/12 检索增补(写 related work 时必须处理,出处见 v3 审阅 §5.2 与综合审阅 §2.3)**:

- **Cybernetica ZK-PoL(arXiv:2506.16812, ARES 2025)**:钉为 Baseline Z 的现代具名实例——2025 年 zk 车辆税务合规的 SOTA 证的是属地谓词,不绑定逐段 tariff 算术、物理可行性与结算总额。
- **Context-Binding Gaps in Stateful ZK Proximity Proofs(arXiv:2604.03900)**:谓词证明缺结算上下文绑定的第三方佐证。
- **IEEE V2X ZKP-ETC(2022)**:challenge-response 通行验证,归入谓词层。
- **Zone-based MPC energy billing(arXiv:2307.08778)与 carbon claims(arXiv:2506.16347)**:归入 verifiable metered billing 谱系一句话处理。
- **检索协议声明句**(正文必须有):"we systematically searched IACR eprint / PoPETs / top-4 proceedings 2020-2026 for privacy-preserving road pricing; the most recent system-level advance is P4TC (PoPETs 2020)" + KIT survey 分类自检。
- **completeness 历史重读**(第七条 motivation 线):spot-check 谱系的路侧基建买的主要是 completeness 而非 correctness;WayBill 把两维分离——correctness 由 settlement proof,completeness 由月度 odometer 对账(§3.3),路侧基建在两维上都不再必要。建议配 correctness × completeness 2×2 框架图(四谱系各占一格,WayBill 独占 proof+reconciliation 格)。
...
## 20. 当前已知问题与优先级(2026-06-12 全面刷新)

**历史 P0/P1/P2(2026-06-10 列出的 10 项)已全部关闭**,关闭记录散见 §21;本节只保留当前真实状态。

### 已全部关闭的历史项(速查)

| 历史项 | 关闭方式 |
|---|---|
| Proof-only 提交路径 | `/settlement/period/proof-only` + 负例测试(§7.1) |
| E3 residual runner + 离线扩展 | 四数据集全量 + 50m bucket 敏感性(§12.3.1) |
| Rapidsnark 实测 | median 1.389s measured(§15.1.1) |
| E4 spot-check / 真路网 | 双宇宙 + 检出曲线 + Stackelberg + R/T/Z(§13.4-13.8) |
| E5 泄漏分析 | anonymity + 光谱 + bucketing + 仲裁泄漏(§14.3-14.6) |
| 仓库卫生 / LFS / archived 标注 / README | 分组 commit + git-lfs + `README_ARTIFACT.md` |
| 选择性不提交(v3 GAP-1,2026-06-12 发现) | 月度 odometer 对账(§3.3),2-3 天内修复并测试 |

### 当前 P0:论文写作(唯一阻塞项)

实验与工程无任何已知缺口,所有素材就绪。写作顺序建议:Intro(五线交汇 + 能力勘误 + completeness 重读)→ formal model(§17.5)→ security analysis(§17.6)→ E0-E6(素材全在 §9.5-§15)→ related work(§18.1 六节 + 检索增补)。三张建议主图:correctness×completeness 2×2、enforcement-privacy Pareto、ε 防御纵深瀑布(数据全部已有,只差画图)。

### 当前 P1:投稿前写作动作(各 ≤0.5 天)

1. 检索协议声明句 + KIT survey 分类自检段(§18.1 增补)。
2. ε-economic soundness 文献定位段(covert adversaries / rational protocol design / zk mechanisms,v3 审阅 §4.1 有表)。
3. Cybernetica ZK-PoL 钉为 Baseline Z 代表 + 三类相邻工作各一句(§18.1 增补)。
4. E4 美元锚(NYC $555M cordon 合同)+ Singapore ERP 2.0 / Smart Tacho LCV 扩张进 motivation(v3 审阅 §3)。
5. cadence dial 三轴表(E3 0.187/E5 元组数/E6 208s,数字全有)。

### 当前 P2:camera-ready / future work(不阻塞投稿)

1. **Deferred-disclosure settlement**(v6 电路分支):running-total 承诺链,期中只见承诺、月末开总额;正文给设计草图 + 用现有 E5 数据做匿名度增益分析估计(v3 审阅 §6.4 有完整方案)。**不动已封印 artifact**。
2. Sparse tariff / larger footprint(解出租车产出,需重 trusted setup)。
3. spot-check 观测加时间戳维度(现 segment-only 偏向 baseline;加了结论只会更强)。
4. E0 多 zone footprint 扩展(对 Rome 官方多边形真值测 cell-boundary 误差)。
5. 跨月不可链接性(per-domain pseudonymous attestation key)。
6. IVC/folding 流式证明(构造草图见 §19.5)。
7. PoPETs 2026 各期 / NDSS·Oakland 2027 放榜时文献扫尾。
...
9. [x] **E4 generous baseline / deterrence**
   ...
   - 真实 PBF graph loader 当时未完成;**2026-06-11 已落地为 §13.5(margin-0 四城)**。
...
16. [x] **仓库卫生**(2026-06-11 完成:分组干净 commit、git-lfs 跟踪 zk 产物、指纹记录于 `README_ARTIFACT.md`)
...
### 21.5 完成台账:2026-06-11/12 增量(原清单之外的全部新增工作)

**协议/工程**:

- [x] WayBill 命名定案与映射规则(§0)。
- [x] **月度 odometer 对账修复选择性不提交**(v3 GAP-1):attestation 接口 + `/month/attest`、`/month/{m}/reconcile`、monthly revenue soundness proposition(§3.3);`(device_id, period_id)` 键、分设备账本、时间窗不重叠(GAP-2/3);charger 测试 10→14。
- [x] 争议仲裁 opening verifier(`common/dispute.py`,14 测试)+ 泄漏量化(§14.6)。
- [x] 仓库卫生:分组 commit、git-lfs、`README_ARTIFACT.md`、appendix 素材文档、rapidsnark 源码编译。
- [x] GeoLife 地理锚点复原(2026-06-12,§9.5):内置北京中心网格,逐字节验证——所有"GeoLife 无锚点"限制作废。

**实验(全部有 receipt)**:

- [x] E0 三数据集(§9.5,trip 流式含 Porto 1.7GB zip)。
- [x] E1 tariff-spread 曲线(§10.4.1)。
- [x] E2/E3 四数据集扩展(§11.3.1/§12.3.1)+ E3 50m bucket 敏感性。
- [x] E4:margin-0 真路网四城(§13.5)、检出曲线/等检出工作点/R/T/Z head-to-head(§13.6)、Baseline R 经验仿真 + Stackelberg 双最优反应(§13.7)、交叉对称评估(§13.8)。
- [x] E5:泄漏光谱(§14.4)、statement-bucketing(§14.5)、仲裁泄漏(§14.6)、GeoLife 补全。
- [x] E6 rapidsnark 实测(§15.1.1)。

**审计修复(方法论三训的出处)**:E1 objective drift(历史)、TEE 微基准误含 Poseidon(216.5→0.044ms)、Baseline R 分母漂移(移动段→全账单)、drivable margin 反向偏置(200m→0)、E0 尾段区间错配、GeoLife 等检出工作点细网格(370→350)、anonymity 池对齐(123 同池)。全部记录在两份综合审阅(06-11、含 §九)与 v3 审阅 §9。

**配套文档**:`TSIP_RUC_综合审阅_2026-06-11.md`(baseline 裁定 + 六项创新建议)、`TSIP_RUC_综合审阅_v3_2026-06-11.md`(GAP-1 + thesis 修订 + 新文献,含执行状态表)、`TSIP_RUC_论文附录素材_2026-06-11.md`、`README_ARTIFACT.md`。
