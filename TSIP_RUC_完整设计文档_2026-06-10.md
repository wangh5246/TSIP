...
- **E3 已完成全量 residual sweep**：`script/run_e3_relay_residual.py` 首版结果在 `experiments/e3_relay_residual/`；2026-06-11 全量离线扩展（Rome 162→66 moving periods、radius 至 400m）在 `experiments/e3_relay_residual_full/`，最坏格点 Rome fine/intra_cell/400m p95 residual `0.507`。定位为 bucketed RCSPP upper-bound；E4/E5 仍需按外部 baseline 和泄漏分析口径继续完善。
...
| E3 | proof-consistent relay / adversarial residual | 已完成首版 residual sweep，见 `experiments/e3_relay_residual/` |
| E4 | 路侧基础设施成本 vs spot-check baseline | 设计明确，未跑正式数字 |
| E5 | 隐私泄漏 vs GPS-upload | baseline 口径明确，未正式量化 |
| E6 | 性能与规模 | 单 period + direct accumulation 成本已有 |
...
已用 runner 默认全量口径重跑：Rome 全部 162 periods 经 `min_distance_m=500` moving 过滤后得 66 periods，relay radius 扩展到 `{0,100,200,400}`，cadence `{60,120,300}` × granularity `{coarse,medium,fine}` × payload `{cell,intra_cell}` 共 144 个 summary 格点、5760 period rows。输出目录：`experiments/e3_relay_residual_full/`（receipt 含输入与参数）。首版 `experiments/e3_relay_residual/` 保留为 first-version 样本,引用时以全量目录为准。

| Dataset | Payload mode | max p95 residual | max mean residual | max residual | 最坏格点 |
|---|---|---:|---:|---:|---|
| GeoLife | cell | 0.212466 | 0.161637 | 0.226006 | coarse / 60s / 400m |
| GeoLife | intra_cell | 0.279353 | 0.193741 | 0.281010 | coarse / 60s / 400m |
| Rome (66 moving) | cell | 0.373862 | 0.251869 | 0.407150 | fine / 120s / 400m |
| Rome (66 moving) | intra_cell | 0.507394 | 0.347975 | 0.535751 | fine / 120s / 400m |
...
> Under a bucketed RCSPP upper-bound model, proof-consistent relay/meaconing residuals remain sensitive to relay radius, tariff granularity, and whether intra-cell payload remainder is exposed. On the first GeoLife/Rome moving-period sweep, GeoLife stays below p95 residual 0.172 even under intra-cell relay, while the Rome moving subset reaches p95 0.402 in the coarse/intra-cell/200m setting.
...
- rapidsnark 外推：单 period prove 约 0.6s（**extrapolated；当前本地未安装 `rapidsnark` / `prover`，不能作为实测数字引用**）

### 15.2 月度 direct accumulation 估算

按每天驾驶 2h、每月 30 天：

| cadence | proofs/month | snarkjs client prove measured | rapidsnark client prove extrapolated | server verify measured |
|---|---:|---:|---:|---:|
| 300s | 30 | ~214-262s | ~18s | ~7s |
| 120s | 75 | ~535-654s | ~45s | ~17s |
| 60s | 150 | ~1,070-1,308s | ~90s | ~34s |

2026-06-11 check：`which rapidsnark` 和 `which prover` 均未找到；`experiments/e4_e5_ruc/receipt.json` 也记录 `rapidsnark=null`、`rapidsnark_prover=null`。因此论文正文只能把 `snarkjs` 作为 measured result，rapidsnark 只能标注为 extrapolated / modeled optimization。
...
| `experiments/e3_relay_residual_full/` | E3 全量权威输出（首版 `e3_relay_residual/` 保留为样本） |
...
2. **E3 residual runner**
   ...
   - 离线扩展已完成（2026-06-11）：Rome 全量 162 periods（66 moving）、400m relay radius，见 §12.3.1 / `experiments/e3_relay_residual_full/`。更细 distance bucket 仍为可选增强。

3. **Rapidsnark 实测**
   - 2026-06-11 本地未找到 `rapidsnark` / `prover`。
   - 已在 §15.2 统一标注 rapidsnark 为 `extrapolated`；正文只把 snarkjs 作为 measured result。
...
2. [x] **Rapidsnark 实测或统一标注 extrapolated**
   - 2026-06-11：`which rapidsnark` / `which prover` 均未找到；`experiments/e4_e5_ruc/receipt.json` 记录 `rapidsnark=null`。
   - §15.2 已把月度表改成 `snarkjs measured` + `rapidsnark extrapolated`，正文只把 snarkjs 作为实测。
