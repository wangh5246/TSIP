...
// Settlement v6 period circuit with explicit position-validity.
// C1-C5 keep continuity, C11-C16 keep ADWC, C17-C20 bind grid cells to
// tariff zones, C21 binds odometer deltas and explicit position validity to the fee, and C22
...
template SettlementPeriodV6(N_FIXES, TREE_DEPTH, K_WINDOW) {
    ...
    signal input policy_profile_commitment;
    ...
    // One explicit bit per interval.  This bit is committed in the public
    // interval root; it is never inferred from elapsed time.
    signal input position_valid[N_FIXES - 1];
    ...
    component dtPositive[N_FIXES - 1];
    ...
    component zoneRateRange[N_FIXES - 1];
    component zoneRateMaxLE[N_FIXES - 1];
    component zonePathHash[N_FIXES - 1][TREE_DEPTH];
    component normalFeeRange[N_FIXES - 1];
    component fallbackFeeRange[N_FIXES - 1];
    component feeTermRange[N_FIXES - 1];
    ...
    component tierVmaxSqRange;
    component modeVmaxSqRange;
    component capPolicyRange;
    component maxZoneRateRange;
    component maxDtRange;
    component cadenceRange;
    component spanRange;
    component spanMaxLE;
    ...
    for (var i = 0; i < N_FIXES; i++) {
        ...
        fixCommit[i].in[3] <== i;
        fixCommit[i].in[4] <== auth_time[i];
        fixCommit[i].in[5] <== fix_cell_x[i];
    ...
    tierVmaxSqRange = Num2Bits(32);
    tierVmaxSqRange.in <== tier_vmax_sq;
    modeVmaxSqRange = Num2Bits(32);
    modeVmaxSqRange.in <== mode_vmax_sq;
    capPolicyRange = Num2Bits(80);
    capPolicyRange.in <== cap_policy_sq;
    maxZoneRateRange = Num2Bits(32);
    maxZoneRateRange.in <== max_zone_rate_cents_per_m;
    maxDtRange = Num2Bits(16);
    maxDtRange.in <== max_dt_sec;
    cadenceRange = Num2Bits(16);
    cadenceRange.in <== cadence_sec;
    ...
    for (var i = 0; i < N_FIXES - 1; i++) {
        ...
        dtPositive[i] = LessThan(16);
        odoDeltaRange[i] = Num2Bits(64);
        dtRange[i].in <== dt[i];
        dtPositive[i].in[0] <== 0;
        dtPositive[i].in[1] <== dt[i];
        dtPositive[i].out === 1;
        ...
        zoneRateRange[i] = Num2Bits(32);
        zoneRateRange[i].in <== zone_rate_cents_per_m[i];
        zoneRateMaxLE[i] = LessEqThan(32);
        zoneRateMaxLE[i].in[0] <== zone_rate_cents_per_m[i];
        zoneRateMaxLE[i].in[1] <== max_zone_rate_cents_per_m;
        zoneRateMaxLE[i].out === 1;
        ...
        selected_normal_fee[i] <== normal_fee[i] * position_valid[i];
        selected_fallback_fee[i] <== fallback_fee[i] * fallback_flag[i];
        fee_terms[i] <== selected_normal_fee[i] + selected_fallback_fee[i];
        normalFeeRange[i] = Num2Bits(96);
        normalFeeRange[i].in <== normal_fee[i];
        fallbackFeeRange[i] = Num2Bits(96);
        fallbackFeeRange[i].in <== fallback_fee[i];
        feeTermRange[i] = Num2Bits(96);
        feeTermRange[i].in <== fee_terms[i];
