...
def threshold_metrics(y_true: np.ndarray, scores: np.ndarray, threshold: float = 0.5) -> Dict[str, float]:
    ...
    return {
        ...
        "tpr_at_0_5": float(tp / max(1.0, tp + fn)),
        "fpr_at_0_5": float(fp / max(1.0, fp + tn)),
    }


def write_feature_summary(out_path: Path, x: np.ndarray, y: np.ndarray) -> None:
...
def write_predictions(
    ...
    with out_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["uid", "target_round", "label", "score"])
        writer.writeheader()
...
def main(argv: Sequence[str]) -> int:
    ...
    x, y, metas = build_dataset(participated, submissions, warmup_rounds)
    ...
    write_feature_summary(out_dir / "feature_summary.csv", x, y)
