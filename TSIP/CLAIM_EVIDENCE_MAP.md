...
| CE07 | At fixed epsilon 5, three-seed N=1000 SHTPC mean Jaccard exceeds the mechanism-level external ESA adapter on all five datasets, all 15 observed dataset-seed differences favor SHTPC, and SHTPC has FRR 0 and MRR 1. | Abstract; RQ5 | `docker_v4_smoke/utility/three_seed_n1000/fair_fixed_comparison.json`; `run_v4_utility_compare.py`; `baselines/nebula_baseline.py`; generated table and Figure 5 | supported | Retain all five deltas. State SHTPC threshold 2 separately from the adapter's raw-count threshold 3 and sampling/debiasing rule. Make no statistical-significance claim for the small Porto and Synthetic differences. |
...
| E02a | Experimental Setup | The RQ5 comparison uses a generic external ESA adapter and an EIFFeL-style adapter. | Specify the ESA sampling, shuffling, raw-count threshold 3, debiasing, and no-trajectory-rejection rules while distinguishing mechanism emulation from native deployment. | CE07 |
...
| E20 | RQ5 | SHTPC has five positive mean Jaccard deltas over the external ESA adapter under the fixed protocol. | Report every absolute value and all 15 observed positive paired differences while withholding statistical-significance claims for the three-seed small gains. | CE07, CE20 |
