...
The V11 config retains the V3 S2/S3/S5 repairs and pins
...
to the annotated `waybill-formal-readiness-v11` tag. The self-verifying release
is stored at `artifacts/releases/waybill-formal-readiness-v11`. V11 retains the
...
Btrfs run root to be beneath `/work`. V11 also separates the 25-fix raw
submission cap from the signed proof-only attestation envelope, whose bounded
count covers the frozen matrix through 200 and remains profile-equality
checked. It accepts only the exact pre-registered underscore-form M2 wrapper
identifiers in addition to the production hyphen-form V6 namespace. Run the
...
python script/finalize_waybill_release.py preflight \
  --tag waybill-formal-readiness-v11 \
  --config configs/waybill_formal/release-v11.json \
  --output-manifest artifacts/release-inputs-v11/preflight.json

docker save --output artifacts/release-inputs-v11/formal-runner-image.tar \
  waybill-formal:readiness-v11
python script/finalize_waybill_release.py image-metadata \
  --image artifacts/release-inputs-v11/formal-runner-image.tar \
  --image-ref waybill-formal:readiness-v11 \
  --role formal-runner \
  --config configs/waybill_formal/release-v11.json \
  --preflight artifacts/release-inputs-v11/preflight.json \
  --output artifacts/release-inputs-v11/formal-runner-metadata.json
docker run --rm \
  -v "$PWD/artifacts/release-inputs-v11:/out" \
...
python script/finalize_waybill_release.py bind-sbom \
  --input artifacts/release-inputs-v11/formal-runner-raw.spdx.json \
  --metadata artifacts/release-inputs-v11/formal-runner-metadata.json \
  --role formal-runner \
  --preflight artifacts/release-inputs-v11/preflight.json \
  --output artifacts/release-inputs-v11/formal-runner-sbom.spdx.json
docker run --rm \
  -v "$PWD/artifacts/release-inputs-v11:/out" \
  aquasec/trivy:0.72.0@sha256:cffe3f5161a47a6823fbd23d985795b3ed72a4c806da4c4df16266c02accdd6f \
...
python script/finalize_waybill_release.py bind-vulnerability-report \
  --input artifacts/release-inputs-v11/formal-runner-raw.trivy.json \
  --metadata artifacts/release-inputs-v11/formal-runner-metadata.json \
  --role formal-runner \
  --config configs/waybill_formal/release-v11.json \
  --preflight artifacts/release-inputs-v11/preflight.json \
  --output artifacts/release-inputs-v11/formal-runner-vulnerabilities.trivy.json
```
...
python script/finalize_waybill_release.py verify \
  --release artifacts/releases/waybill-formal-readiness-v11
```
