...
def render_tex(summary: dict[str, Any]) -> str:
    lines = [
        ...
        r"\begin{table}[t]",
        r"  \centering",
        rf"  \caption{{Manifest-bound Groth16 execution at $N{{=}}1000$ over three seeds. All functional receipts contribute to proof totals. Timing excludes units with an inter-round gap above {int(TIMING_GAP_LIMIT_SECONDS):,} seconds; entries report mean $\pm$ sample standard deviation over the remaining timing receipts.}}",
