...
from common.proof_payload import (
    ...
    poseidon_chain,
    sample_reconstructible_share_pair,
)
...
EXPERIMENT_ROUND_INDEX = int(os.getenv("EXPERIMENT_ROUND_INDEX", "0"))
...
#   "share_swap"                  - mutate an A-channel share after proving (C21 negative fixture)
#   "replay"                      - replay the previous accepted wire report and proof in a new round
#   "prev_tamper"                 - mutate the predecessor commitment after proving
#   "anchor_tamper"               - mutate the ADWC anchor commitment after proving
...
TSIP_WASM = os.getenv(
    ...
    _first_existing_path(
        [
            "/app/v4/zk/v4_paper_k6/tsip_main_v4_paper_k6_js/tsip_main_v4_paper_k6.wasm",
            "zk/v4_paper_k6/tsip_main_v4_paper_k6_js/tsip_main_v4_paper_k6.wasm",
        ]
    ),
)
TSIP_ZKEY = os.getenv(
    ...
    _first_existing_path(
        [
            "/app/v4/zk/v4_paper_k6/tsip_main_v4_paper_k6_final.zkey",
            "zk/v4_paper_k6/tsip_main_v4_paper_k6_final.zkey",
        ]
    ),
)
...
POST_ROUND_WAIT_SEC = float(os.getenv("POST_ROUND_WAIT_SEC", "5"))
SAFE_PROTOCOL_TRAJECTORY = os.getenv("TSIP_SAFE_PROTOCOL_TRAJECTORY", "0") == "1"
CLIENT_TOTAL = int(os.getenv("CLIENT_TOTAL", "200"))
TSIP_USER_SCOPE = os.getenv("TSIP_USER_SCOPE", "stable").strip().lower()
CLIENT_USER_NAMESPACE = os.getenv("CLIENT_USER_NAMESPACE", "").strip()
TRAJ_SOURCE = os.getenv("CLIENT_TRAJ_SOURCE", "synthetic").strip().lower()
REAL_TRAJ_SOURCES = {"tdrive", "geolife", "porto", "rome", "synthetic"}
...
def safe_protocol_trajectory(user_id: str, length: int = 32):
    """Create bounded, timestamped points for protocol-only smoke tests.

    Utility experiments consume the original dataset windows. This path only
    isolates protocol plumbing from dataset-specific jumps/timestamp quality.
    """
    base = _stable_u64("v4-safe-protocol", user_id)
    base_x = 1000 + int(base % 8000)
    base_y = 1000 + int((base >> 16) % 8000)
    points = []
    for index in range(max(32, int(length))):
        points.append(
            (
                float(base_x + (index % 16) * 5),
                float(base_y + (index % 16) * 3),
                int(1_700_000_000 + index * max(1, TSIP_WINDOW_SEC)),
            )
        )
    return points
                ...
                out.append(
                    {
                        ...
                        "loc_salt": int(it.get("loc_salt", 0) or 0),
                    }
                )
            except Exception:
                continue
        ...
        return [
            {
                ...
                "loc_salt": int(prev_state.get("loc_salt", 0) or 0),
            }
        ]
    return []
...
def _recover_tsip_state_from_server(
    ...
    """Fail closed when a client has lost its private randomized openings.

    Location salts are independent CSPRNG witnesses.  The server deliberately
    cannot reconstruct them from a trajectory, uid, or timestamp; deployments
    must restore the client's protected local state backup before resuming a
    chain.  Returning a failure is preferable to silently recreating predictable
    openings that would invalidate the hiding claim.
    """
    del user_id, recovery, trajectory
    return None, "local randomized commitment state is required; restore protected client state"

    """Unreachable legacy recovery implementation retained below temporarily."""
    ...
    for j, p in enumerate(trajectory):
        try:
            ...
            loc_salt_i, loc_field_i = location_opening(
                user_id=user_id,
                secret_hex=recovered_secret,
                x=x_i,
                y=y_i,
                timestamp=t_i,
            )
            loc_i = f"{loc_field_i:064x}"
            points.append(
                {
                    "traj_idx": int(j),
                    "x": int(x_i),
    ...
    recovered_state = {
        ...
        "loc_salt": int(chosen_curr["loc_salt"]),
    ...
    out.append(
        {
            ...
            "x": int(x),
            "y": int(y),
            "loc_commitment": str(loc_commitment),
            "loc_salt": int(loc_salt),
        }
    )
...
def build_tsip_payload(
    ...
    """Build a V4 paper-profile payload bound to both routed share packages."""
    if not TSIP_ENABLE:
        return None, None, {}

    payload_idx = idx if idx is not None else []
    payload_val = val if val is not None else []
    if not submission_id or int(round_id) <= 0:
        raise RuntimeError("v4-paper requires round_id and submission_id")
    if len(payload_idx) != KPRIME or len(payload_val) != KPRIME:
        raise RuntimeError("v4-paper requires a complete fixed-layout report")
    if PRP_ENABLE:
        raise RuntimeError("v4-paper same-primary binding currently requires PRP_ENABLE=0")
    ...
    def _build_report_binding(
        ...
        cell_x = int(curr_x) // int(CELL_SIZE_M)
        cell_y = int(curr_y) // int(CELL_SIZE_M)
        rem_x = int(curr_x) - cell_x * int(CELL_SIZE_M)
        rem_y = int(curr_y) - cell_y * int(CELL_SIZE_M)
        primary_idx = cell_y * int(GRID_W) + cell_x
        if not (0 <= primary_idx < DOMAIN):
            raise RuntimeError(f"v4-paper primary out of range: {primary_idx}")

        if primary_idx in payload_idx:
            source = payload_idx.index(primary_idx)
            payload_idx[0], payload_idx[source] = payload_idx[source], payload_idx[0]
            payload_val[0], payload_val[source] = payload_val[source], payload_val[0]
        else:
            payload_idx[0] = int(primary_idx)
        payload_val[:] = [int(contribution_bit)] + ([0] * (KPRIME - 1))
        if len(set(int(item) for item in payload_idx)) != KPRIME:
            raise RuntimeError("v4-paper primary insertion created duplicate indices")

        canonical_pairs = sorted(
            (int(position), int(value))
            for position, value in zip(payload_idx, payload_val)
        )
        payload_idx[:] = [position for position, _value in canonical_pairs]
        payload_val[:] = [value for _position, value in canonical_pairs]

        val_a, val_r = secret_share_vals(payload_val)
        ...
        context_commitment = int(canonical_digest_field(context))
        ...
        if not validate_fingerprint_relation(
            ...
            raise RuntimeError("v4-paper share fingerprints do not reconstruct canonical payload")
        binding_fields = derive_paper_binding_fields(
        ...
        curr_loc_salt, curr_loc_field = location_opening(
            user_id=user_id,
            secret_hex=user_secret,
            x=int(curr_x),
            y=int(curr_y),
            timestamp=int(curr_t),
        )
        return {
            "blob_hash": int(binding_fields["blob_hash"]),
            "context": context,
            "context_commitment": context_commitment,
            "curr_loc_commitment": f"{curr_loc_field:064x}",
            "curr_loc_salt": int(curr_loc_salt),
            "payload_cell_x": int(cell_x),
            "payload_cell_y": int(cell_y),
            "payload_commitment_v2": int(binding_fields["payload_commitment_v2"]),
            "payload_digest": str(payload_digest),
            "payload_primary_idx": int(primary_idx),
            "payload_rem_x": int(rem_x),
            "payload_rem_y": int(rem_y),
            "contribution_bit": int(contribution_bit),
            "fingerprint_a": fingerprint_a,
            "fingerprint_challenge": alpha,
            "fingerprint_r": fingerprint_r,
            "package_digest_a": package_digest_a,
            "package_digest_r": package_digest_r,
            "primary_commitment": int(binding_fields["primary_commitment"]),
            "primary_salt": primary_salt,
            "share_content_commitment_a": int(binding_fields["share_content_commitment_a"]),
            "share_content_commitment_r": int(binding_fields["share_content_commitment_r"]),
            "val_a": val_a,
            "val_r": val_r,
        }
    ...
    def _build_enrollment_payload(
        ...
        binding = _build_report_binding(curr_x, curr_y, curr_t, next_epoch, 0)
        curr_loc_commitment = str(binding["curr_loc_commitment"])
        curr_loc_salt = int(binding["curr_loc_salt"])
        ...
        payload = {
            ...
            "payload_commitment_field": str(binding["payload_commitment_v2"]),
            "payload_commitment_v2": str(binding["payload_commitment_v2"]),
            "primary_commitment": str(binding["primary_commitment"]),
            "share_content_commitment_a": str(binding["share_content_commitment_a"]),
            "share_content_commitment_r": str(binding["share_content_commitment_r"]),
            "contribution_bit": int(binding["contribution_bit"]),
            "fingerprint_a": str(binding["fingerprint_a"]),
            "fingerprint_challenge": str(binding["fingerprint_challenge"]),
            "fingerprint_r": str(binding["fingerprint_r"]),
            "package_digest_a": str(binding["package_digest_a"]),
            "package_digest_r": str(binding["package_digest_r"]),
            "context": binding["context"],
            "context_commitment": str(binding["context_commitment"]),
            "blob_hash": str(binding["blob_hash"]),
    ...
    def _build_followup_payload(
        ...
        epoch = int(prev_s.get("enrollment_epoch", 1) or 1)
        contribution_bit = int(
            int(prev_s.get("warmup_count", 0) or 0) >= int(TSIP_ADWC_WINDOW_K)
        )
        binding = _build_report_binding(
            curr_x,
            curr_y,
            curr_t,
            epoch,
            ...
            history = [
                {
                    ...
                    "loc_salt": int(prev_s.get("loc_salt", 0) or 0),
                }
            ]
        ...
        inputs = {
            ...
            "r_prev": int(prev_s.get("loc_salt", 0) or 0),
            "r_curr": int(curr_loc_salt),
            "r_anchor": int(anchor_state.get("loc_salt", 0) or 0),
            ...
            "payload_cell_x": int(binding["payload_cell_x"]),
            "payload_cell_y": int(binding["payload_cell_y"]),
            "payload_rem_x": int(binding["payload_rem_x"]),
            "payload_rem_y": int(binding["payload_rem_y"]),
            "payload_primary_idx": int(binding["payload_primary_idx"]),
            "primary_salt": int(binding["primary_salt"]),
            "primary_commitment": int(binding["primary_commitment"]),
            "payload_commitment_v2": int(binding["payload_commitment_v2"]),
            "contribution_bit": int(binding["contribution_bit"]),
            "fingerprint_a": int(binding["fingerprint_a"]),
            "fingerprint_challenge": int(binding["fingerprint_challenge"]),
            "fingerprint_r": int(binding["fingerprint_r"]),
            "package_digest_a": int(binding["package_digest_a"]),
            "package_digest_r": int(binding["package_digest_r"]),
            "share_content_commitment_a": int(binding["share_content_commitment_a"]),
            "share_content_commitment_r": int(binding["share_content_commitment_r"]),
            "context_commitment": int(binding["context_commitment"]),
            "blob_hash": int(binding["blob_hash"]),
        ...
        meta = {
            ...
            "proof_attempted": TSIP_VERIFY_MODE == "full",
            "proof_generated": isinstance(proof_data.get("proof"), dict),
            "val_a": binding["val_a"],
            "val_r": binding["val_r"],
    ...
    if trajectory:
        ...
        if malicious and ATTACK_TYPE == "boundary_teleport":
            meta["attack_step_distance_m"] = round(
                math.hypot(float(curr_x - prev_x), float(curr_y - prev_y)), 3
            )
            meta["admissible_step_m"] = round(
                math.sqrt(float(declared_tier_vmax_sq)) * float(time_diff), 3
            )
        if malicious and ATTACK_TYPE == "gradual_drift":
...
def secret_share_vals(val):
    ...
    valR = []
    for v in val:
        a, b = sample_reconstructible_share_pair(
            clear_value=int(v),
            mask_bits=MASK_BITS,
        )
        valA.append(a)
...
def main():
    ...
    tsip_proof_attempts = 0
    tsip_proof_generated = 0
    ...
    for i in range(report_total):
        ...
        if TRAJ_SOURCE in REAL_TRAJ_SOURCES:
            ...
            trajectory = None
        if CLIENT_USER_NAMESPACE:
            user_id = f"{CLIENT_USER_NAMESPACE}_{user_id}"
        if SAFE_PROTOCOL_TRAJECTORY:
            trajectory = safe_protocol_trajectory(user_id)
        ...
        if TSIP_ENABLE:
            ...
            if is_malicious and ATTACK_TYPE == "replay" and prev_tsip is not None:
                candidate = prev_tsip.get("last_report")
                if isinstance(candidate, dict):
                    replay_source = json.loads(json.dumps(candidate))
            ...
            valA = list(tsip_meta.get("val_a", valA))
            valR = list(tsip_meta.get("val_r", valR))
            tsip_proof_attempts += int(bool(tsip_meta.get("proof_attempted", False)))
            tsip_proof_generated += int(bool(tsip_meta.get("proof_generated", False)))
        ...
        if TSIP_ENABLE:
            ...
            if next_tsip_state is not None:
                next_tsip_state["last_report"] = {
                    "round_id": int(round_id),
                    "submission_id": str(submission_id),
                    "idx": list(idx),
                    "val_a": list(valA),
                    "val_r": list(valR),
                    "tsip": json.loads(json.dumps(tsip_payload)),
                }
        if is_malicious and ATTACK_TYPE == "share_swap":
            reportA["val"] = list(reportA["val"])
            reportA["val"][0] = (int(reportA["val"][0]) + 1) & MASK
        elif is_malicious and ATTACK_TYPE == "replay" and replay_source is not None:
            reportA = {
                "round_id": int(replay_source["round_id"]),
                "submission_id": str(replay_source["submission_id"]),
                "idx": list(replay_source["idx"]),
                "val": list(replay_source["val_a"]),
                "tsip": json.loads(json.dumps(replay_source["tsip"])),
            }
            reportR = {
                "round_id": int(replay_source["round_id"]),
                "submission_id": str(replay_source["submission_id"]),
                "idx": list(replay_source["idx"]),
                "val": list(replay_source["val_r"]),
                "tsip": json.loads(json.dumps(replay_source["tsip"])),
            }
        elif is_malicious and ATTACK_TYPE == "prev_tamper" and reportA.get("tsip"):
            reportA["tsip"]["prev_loc_commitment"] = tamper_commitment_hex(
                str(reportA["tsip"].get("prev_loc_commitment", ""))
            )
        elif is_malicious and ATTACK_TYPE == "anchor_tamper" and reportA.get("tsip"):
            reportA["tsip"]["anchor_loc_commitment"] = tamper_commitment_hex(
                str(reportA["tsip"].get("anchor_loc_commitment", ""))
            )

        route_a_started_ns = time.perf_counter_ns()
        ok_a, data_a, status_a = post_report(SHUFFLER_URL_A, reportA)
        route_a_elapsed_ms = (time.perf_counter_ns() - route_a_started_ns) / 1_000_000.0
        route_r_started_ns = time.perf_counter_ns()
        ok_r, data_r, status_r = post_report(SHUFFLER_URL_R, reportR)
        route_r_elapsed_ms = (time.perf_counter_ns() - route_r_started_ns) / 1_000_000.0
        ...
        if accepted:
            ...
            rejection_reason = detail_a or detail_r or "unknown"
            reject_reason_counter[rejection_reason] += 1
        if is_malicious:
            malicious_total += 1
            if not accepted:
        ...
        cursor = int(next_tsip_state.get("cursor", -1)) if isinstance(next_tsip_state, dict) else -1
        true_primary = None
        if isinstance(record, dict):
            windows = record.get("windows", [])
            if isinstance(windows, list) and 0 <= cursor < len(windows):
                try:
                    true_primary = int(windows[cursor]["cell_id"])
                except (KeyError, TypeError, ValueError):
                    true_primary = None
        submitted_primary = None
        if isinstance(next_tsip_state, dict):
            try:
                submitted_primary = (
                    (int(next_tsip_state["y"]) // int(CELL_SIZE_M)) * int(GRID_W)
                    + (int(next_tsip_state["x"]) // int(CELL_SIZE_M))
                )
            except (KeyError, TypeError, ValueError, ZeroDivisionError):
                submitted_primary = None
        event = {
            "schema_version": "tsip-report-event-v1",
            "logical_round": int(EXPERIMENT_ROUND_INDEX),
            "round_id": int(round_id),
            "report_index": int(i),
            "submission_id": str(submission_id),
            "wire_round_id_a": int(reportA.get("round_id", round_id)),
            "wire_round_id_r": int(reportR.get("round_id", round_id)),
            "wire_submission_id_a": str(reportA.get("submission_id", submission_id)),
            "wire_submission_id_r": str(reportR.get("submission_id", submission_id)),
            "dataset": str(TRAJ_SOURCE),
            "actor_id": str(record.get("actor_id", "")) if isinstance(record, dict) else "",
            "trip_id": str(record.get("trip_id", "")) if isinstance(record, dict) else "",
            "segment_id": str(record.get("segment_id", "")) if isinstance(record, dict) else "",
            "user_id": str(user_id),
            "effective_user_id": str(effective_user_id),
            "attack_type": str(ATTACK_TYPE if is_malicious else "none"),
            "malicious": bool(is_malicious),
            "trajectory_cursor": int(cursor),
            "true_primary": true_primary,
            "submitted_primary": submitted_primary,
            "proof_attempted": bool(tsip_meta.get("proof_attempted", False)),
            "proof_generated": bool(tsip_meta.get("proof_generated", False)),
            "proof_error": str(tsip_meta.get("proof_error", "")),
            "attack_step_distance_m": tsip_meta.get("attack_step_distance_m"),
            "admissible_step_m": tsip_meta.get("admissible_step_m"),
            "route_a_ok": bool(ok_a),
            "route_r_ok": bool(ok_r),
            "route_a_status": status_a,
            "route_r_status": status_r,
            "final_acceptance": bool(accepted),
            "rejection_reason": str(rejection_reason),
            "route_a_elapsed_ms": round(route_a_elapsed_ms, 6),
            "route_r_elapsed_ms": round(route_r_elapsed_ms, 6),
            "report_elapsed_ms": round(
                (time.perf_counter_ns() - report_started_ns) / 1_000_000.0,
                6,
            ),
        }
        print("report_event=" + json.dumps(event, sort_keys=True, separators=(",", ":")))
    ...
    print(
        ...
        "tsip_proof_attempts=", tsip_proof_attempts,
        "tsip_proof_generated=", tsip_proof_generated,
    )
    if rejected > 0:
    ...
    time.sleep(max(0.0, POST_ROUND_WAIT_SEC))
