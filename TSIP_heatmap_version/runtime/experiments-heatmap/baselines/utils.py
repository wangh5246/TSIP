...
def extract_cell_sequence(record: Dict, domain: int = DOMAIN_SIZE) -> List[int]:
    ...
    if windows:
        return [_normalize_cell_id(w["cell_id"], domain) for w in windows]
    ...
    if fixes:
        ...
        for fix in fixes:
            if "cell_id" in fix:
                cell_id = fix["cell_id"]
            elif "cell_idx" in fix:
                cell_id = fix["cell_idx"]
            elif "cell_x" in fix and "cell_y" in fix:
                cell_id = int(fix["cell_y"]) * GRID_W + int(fix["cell_x"])
            else:
                continue
            cells.append(_normalize_cell_id(cell_id, domain))
        return cells
...
def pick_cell_for_round(record: Dict, round_idx: int, domain: int = DOMAIN_SIZE) -> Optional[int]:
    ...
    cells = extract_cell_sequence(record, domain)
    if not cells:
        return None
    return cells[round_idx % len(cells)]
