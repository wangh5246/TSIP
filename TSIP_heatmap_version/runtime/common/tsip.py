...
from common.v4_bindings import poseidon_hash
...
def _poseidon2_hash(x: int, y: int) -> int:
    return int(poseidon_hash(int(x), int(y)))
...
# ── Compatibility helper (unused in the V4-paper ADWC main path) ─────────────
def sliding_window_max_dist_sq(vmax: float, window_rounds: int, round_sec: int) -> int:
    """Legacy cumulative-distance bound helper.

    The V4-paper mainline uses ADWC anchor-distance constraints in-circuit
    (`circuits/tsip_main_v4_paper_k6.circom`) instead of cumulative-sum semantics.
    This helper is retained for callers that need a cumulative-distance bound.
    """
